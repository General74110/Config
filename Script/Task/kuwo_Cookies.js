...
const existingLoginUids = $.getdata("kw_loginUid") ? $.getdata("kw_loginUid").split('@') : [];
const existingLoginSids = $.getdata("kw_loginSid") ? $.getdata("kw_loginSid").split('@') : [];

var notice = "";

if (matchUid && matchUid.length === 2 && matchSid && matchSid.length === 2) {
    const newLoginUid = matchUid[1];
    const newLoginSid = matchSid[1];

    let uidIndex = existingLoginUids.indexOf(newLoginUid);
    if (uidIndex !== -1) {
        // 账号已存在，更新其 loginSid
        existingLoginSids[uidIndex] = newLoginSid;
    } else {
        // 账号不存在，添加新账号
        existingLoginUids.push(newLoginUid);
        existingLoginSids.push(newLoginSid);
    }

    $.setval(existingLoginUids.join('@'), "kw_loginUid");
    $.setval(existingLoginSids.join('@'), "kw_loginSid");
