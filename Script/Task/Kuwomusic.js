...
[Task]
cron "3 6 * * * script-path=https://raw.githubusercontent.com/General74110/Quantumult-X/master/Task/Kuwomusic.js, timeout=3600, tag=酷我音乐刷时长, img-url=https://raw.githubusercontent.com/Semporia/Hand-Painted-icon/master/Social_Media/Bebo.png
...
!(async () => {
  ...
  if (typeof $request !== 'undefined') {
    ...
    for (let i = 0; i < loginUidArr.length; i++) {
      ...
      console.log(`开始处理用户 ${currentLoginUid} 的任务`);
      ...
      message = `【${kuwoNameArr[i] || currentLoginUid}】\n` + 
      `【状态】${successMessage}\n` + 
      `【获得时长】${totalHours} 小时\n` + 
      `【到期时间】${lastExpiryTime}\n`;
...
for (let i = 1; i <= kuwocount; i++) {
  ...
  if (nickname) {
    ...
    $.log(`获取【${nickname}】的Cookie成功`);
    $.msg($.name, '', `【${nickname}】获取Cookies成功`);
  } else {
...
// 看广告
function Task(loginUid, timeout = 0) {
  return new Promise((resolve) => {
